...
export default async function ProvidersLandingPage() {
  ...
  return (
    <div className="min-h-screen app-shell">
      {session?.user ? (
        <Navbar mode="app" userEmail={session.user.email} userName={session.user.name} userImage={session.user.image} />
      ) : (
        <Navbar />
      )}
      <main className="mx-auto max-w-5xl px-4 pb-16 pt-28">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {copy.cards.map((title, index) => (
            <div key={title} className="rounded-2xl border border-white/10 bg-white/5 p-5"><h2 className="font-bold">{title}</h2><p className="mt-2 text-sm text-muted">{copy.cardCopy[index]}</p></div>
          ))}
        </div>
